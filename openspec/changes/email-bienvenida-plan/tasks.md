...
- [ ] 1.2 Ejecutar el seed en el entorno de desarrollo y verificar que la plantilla `bienvenida` se crea en la DB.
...
- [ ] 2.2 Verificar que `sendWelcomeEmail` retorna `{ ok, status, logId }` y registra el envío en `EmailLog` (incluye casos skipped si el cliente no tiene correo o la plantilla está desactivada).
...
- [ ] 4.2 Verificar en `/admin/comunicaciones` (pestaña "Plantillas") que la plantilla `bienvenida` aparece destacada, se puede editar/guardar, activar/desactivar y probar sin errores.

## 5. Verificación final

- [ ] 5.1 Verificar que al asignar un plan a un cliente el correo de bienvenida se envía además de la boleta, y que si falla o está desactivado el plan se contrata igual.
- [ ] 5.2 Ejecutar lint/build (`npm run build` o `npm run lint`) sin errores y revisar `openspec validate` del cambio.
