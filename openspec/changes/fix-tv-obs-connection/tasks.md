...
- [ ] 6.1 Tras el deploy, `docker ps`/health: SRS healthy y `nc -zv <host> 1935` responde
- [ ] 6.2 Test directo: `ffmpeg -f lavfi -i testsrc ... -f flv rtmp://<host>:1935/dj/<streamKey>` → status `live`, AutoDJ detenido; al cortar, status `autodj` y AutoDJ reanuda
